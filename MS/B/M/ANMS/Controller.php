...
class Controller extends \App\Http\Controllers\Controller
{
	...
	public function index(){
		...
		return view('ANMS.V.panel_data')->with('data',$data);
	...
	public function indexData(){
		...
		return view('ANMS.V.Object.MasterDetails')->with('data',$data);
...
//dd($NotificationCount);
		...
		if ($data==null or count($dataAll)==$NotificationCount ) {
			...
			'NotificationLink'=>route('ANMS.index.Data'),
	...
	public function viewById($UniqId){
		...
		//dd($UniqId);
		\B\ANMS\Logics::markReadTask($UniqId);


		\MS\Core\Helper\Comman::DB_flush();	
		$m=new Model(1,session('user.userData.UniqId'));
		$m=$m->where('UniqId',$UniqId)->get()->first();
		//dd($m->toArray());
