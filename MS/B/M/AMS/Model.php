...
protected $base_Field;
    ...
    public static function getHireAgencyCode(){
            

        $model=new Model(0);
        $data=$model->pluck('Name','UniqId')->toArray();
        
        $return=[];
        $data2=$model->pluck('Username','UniqId')->toArray();
        $data3=$model->pluck('City','UniqId')->toArray();
        $data4=$model->pluck('State','UniqId')->toArray();
        $data5=$model->pluck('Pincode','UniqId')->toArray();
        
        foreach ($data2 as $key => $value) {
           
                   $return[$key]=  $value.",".$data[$key].",".$data3[$key].",".strtoupper($data4[$key]).",".$data5[$key];
                }        
                return $return;
    ...
    public  function getAgencyCodeFromName($name){
            

        $model=new Model(0);
        $data=$model->where('Name',$name)->first();
        $return=null;
        if($data ==null)  {return $return;}else{ $return=$data->toArray()['UniqId']  ;}
      ...
      public static function getHireAgencyCodeFromId($code){
